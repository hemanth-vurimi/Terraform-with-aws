...
resource "aws_iam_role" "role-for-bastion-host" {
...
resource "aws_iam_policy" "bastion_eks_policy" {
  ...
  description = "Allow bastion to access EKS cluster metadata"
...
resource "aws_iam_role_policy_attachment" "bastion-eks-policy-attach" {
  role       = aws_iam_role.role-for-bastion-host.name
